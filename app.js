...
/**
 * Seepep Results Tracker — Neighbourhoods App (Improved Version)
 * Enhanced with better error handling, validation, accessibility, and performance optimizations
 */
...
function validateFixture(fixture, existingFixtures = []) {
  ...
  if (fixture.homeId === fixture.awayId && fixture.homeId) {
    errors.awayId = "Teams cannot play themselves";
  }
  
  const conflict = existingFixtures.find(f => 
    f.id !== fixture.id &&
    f.date === fixture.date && 
    f.time === fixture.time && 
    f.court === fixture.court
  );
  if (conflict) {
    errors.time = "Time conflict with existing fixture";
  }
...
async function readAll(division) {
...
async function deleteFixtureRemote(id) {
...
function computeLadder(teams, fixtures, results) {
  ...
  fixtures.forEach((f) => {
    ...
    if (r.homeScore > r.awayScore) { 
      home.W++; away.L++; home.PTS += 4; 
    } else if (r.homeScore < r.awayScore) { 
      away.W++; home.L++; away.PTS += 4; 
    } else { 
      home.D++; away.D++; home.PTS += 2; away.PTS += 2; 
    }
  });
  
  Object.values(table).forEach((row) => {
    const denom = row.PA === 0 ? 1 : row.PA;
    row["%"] = Math.round((row.PF / denom) * 1000) / 10;
  });
...
function computeNeighbourhoodTotals(teams, results, fixtures) {
  ...
  fixtures.forEach((f) => {
    const r = resMap.get(f.id); 
    if (!r) return;
    const home = byTeam[f.homeId];
    const away = byTeam[f.awayId];
    if (!home || !away) return;
    
    const hn = home.neighbourhood || "—";
    const an = away.neighbourhood || "—";
    ...
    if (r.homeScore > r.awayScore) { 
      totals[hn].Wins++; totals[an].Losses++; 
    } else if (r.homeScore < r.awayScore) { 
      totals[an].Wins++; totals[hn].Losses++; 
    } else { 
      totals[hn].Draws++; totals[an].Draws++; 
    }
  });
...
function TextInput({ label, value, onChange, placeholder = "", type = "text", className = "", error, required = false, maxLength }) {
  ...
  return (
    <label className={`flex flex-col gap-1 ${className}`} htmlFor={id}>
      ...
      <input
        id={id}
        className={`h-10 rounded-xl border px-3 outline-none focus:ring-2 focus:ring-indigo-500 ${
    ...
    <div className="overflow-auto rounded-xl border border-slate-200">
      <table className="min-w-full text-sm" role="table">
        ...
        <thead className="bg-slate-50 text-slate-600">
          <tr>
            {columns.map((c) => (
              <th key={c.key} className="text-left font-semibold px-3 py-2 whitespace-nowrap" scope="col">
                {c.header}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.length === 0 ? (
            ...
            <tr key={rowKey(row)} className="border-t border-slate-100" role="row">
              {columns.map((c) => (
                <td key={c.key} className="px-3 py-2 align-top whitespace-nowrap" role="gridcell">
                  {c.cell ? c.cell(row, index) : row[c.key]}
                </td>
              ))}
            </tr>
          ))}
...
function TeamsView({ division, data, onUpsertTeam }) {
  ...
  const filtered = useMemo(() => {
    ...
    return src.filter((t) => 
      [t.name, t.coach, t.neighbourhood].some((v) => 
  ...
  const resetDraft = useCallback(() => {
    setDraft({ id: "", name: "", coach: "", neighbourhood: "" });
    setErrors({});
  }, []);
      ...
      <Section title="Add / Edit Team">
        <form className="grid md:grid-cols-4 gap-3" onSubmit={handleSave}>
          ...
          <TextInput 
            label="Coach" 
            value={draft.coach} 
            onChange={(v) => setDraft({ ...draft, coach: v })} 
            error={errors.coach}
            required
            maxLength={50}
          />
          <TextInput 
            ...
            onChange={(v) => setDraft({ ...draft, neighbourhood: v })} 
            maxLength={50}
          />
          <div className="flex items-end gap-2">
            <Button type="submit" loading={loading}>
              {draft.id ? "Update" : "Save"} Team
            </Button>
          ...
          {errors.submit && (
            <div className="md:col-span-4 text-sm text-red-600" role="alert">
              Error: {errors.submit}
            </div>
          )}
        </form>
      ...
      <Section 
        ...
        actions={
          <TextInput 
            ...
            onChange={setQ} 
            className="w-64" 
            placeholder="Search by name, coach, or neighbourhood"
          />
        }
      >
        <Table
          caption={`Teams in ${division} division`}
          columns={[
...
function FixturesView({ division, data, onUpsertFixture, onUpsertResult, onDeleteFixture }) {
  ...
  const resetDraft = useCallback(() => {
    setDraft({ id: "", round: "1", date: "", time: "", court: "", homeId: "", awayId: "" });
    setErrors({});
  }, []);
  ...
  const handleSave = useCallback(async (e) => {
    ...
    const validationErrors = validateFixture(draft, fixtures);
    if (Object.keys(validationErrors).length > 0) {
      setErrors(validationErrors);
      return;
    }
    ...
    try {
      const id = draft.id || uid();
      const f = { ...draft, id, division };
      await onUpsertFixture(f);
      resetDraft();
    } catch (error) {
      setErrors({ submit: error.message });
    } finally {
  ...
  }, [draft, division, fixtures, onUpsertFixture, resetDraft]);

  const saveScore = useCallback(async (id, homeScore, awayScore) => {
    try {
    ...
    <div className="grid gap-6">
      <Section title="Create Fixture">
        <form className="grid md:grid-cols-7 gap-3" onSubmit={handleSave}>
          ...
          {errors.submit && (
            <div className="md:col-span-7 text-sm text-red-600" role="alert">
              Error: {errors.submit}
            </div>
          )}
        </form>
      ...
      <Section title="Fixtures & Results">
        <Table
          caption={`Fixtures and results for ${division} division`}
          columns={[
            ...
            { 
              ...
              header: "Score", 
              cell: (f) => {
                const r = results[f.id] || { homeScore: "", awayScore: "" };
                return (
                  <div className="flex items-center gap-2">
                    <input 
                      className="h-9 w-16 rounded-lg border border-slate-300 px-2 focus:ring-2 focus:ring-indigo-500" 
                      defaultValue={r.homeScore} 
                      placeholder="Home" 
                      onBlur={(e) => saveScore(f.id, e.target.value, r.awayScore)}
                      aria-label={`Home score for ${teams.find(t => t.id === f.homeId)?.name || 'home team'}`}
                      type="number"
                      min="0"
                    />
                    ...
                    <input 
                      ...
                      placeholder="Away" 
                      onBlur={(e) => saveScore(f.id, r.homeScore, e.target.value)}
                      aria-label={`Away score for ${teams.find(t => t.id === f.awayId)?.name || 'away team'}`}
                      type="number"
                      min="0"
                    />
                  </div>
              ...
              cell: (f) => (
                <div className="flex gap-2">
                  <Button 
                    variant="ghost" 
                    onClick={() => setDraft(f)}
                    ariaLabel={`Edit fixture ${f.round}`}
                  >
                    Edit
                  </Button>
                  <Button 
                    variant="danger" 
                    onClick={() => onDeleteFixture(f.id)}
                    ariaLabel={`Delete fixture ${f.round}`}
                  >
                    Delete
                  </Button>
                </div>
...
function ProfilesView({ division, data, onUpsertTeam }) {
  ...
  useEffect(() => {
    if (teams.length && !teams.find(t => t.id === selectedId)) {
  ...
  useEffect(() => {
    if (!team) return;
    setCoach(team.coach || "");
    setRosterText((team.profile?.roster || []).join("\n"));
    setNotes(team.profile?.notes || "");
  }, [team?.id]);
  ...
  const saveProfile = useCallback(async () => {
    ...
    try {
      const updated = {
        ...team,
        coach: sanitizeInput(coach.trim()),
        profile: {
          roster: rosterText.split(/\n+/).map(s => sanitizeInput(s.trim())).filter(Boolean),
          notes: sanitizeInput(notes.trim()),
        },
      };
      await onUpsertTeam(updated);
    } finally {
      setLoading(false);
    }
  }, [team, coach, rosterText, notes, onUpsertTeam]);
    ...
    <div className="grid gap-6">
      ...
      >
        {team ? (
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <label className="flex flex-col gap-1 md:col-span-2">
              ...
              <textarea 
                className="min-h-[200px] rounded-xl border border-slate-300 p-3 focus:ring-2 focus:ring-indigo-500" 
                value={rosterText} 
                onChange={(e) => setRosterText(e.target.value)} 
                placeholder="Player A&#10;Player B&#10;Player C"
                aria-label="Team roster, one player per line"
              />
            </label>
            <label className="flex flex-col gap-1">
              ...
              <textarea 
                ...
                placeholder="Coaching notes, rotations, special instructions…"
                aria-label="Team notes and coaching instructions"
              />
            </label>
          </div>
        ) : (
        ...
        <div className="mt-4 flex gap-2">
          <Button onClick={saveProfile} disabled={!team} loading={loading}>
            Save Profile
          </Button>
        </div>
      </Section>
      ...
      {team && (
        <Section title="Preview">
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <div className="text-slate-500 text-sm">Coach</div>
              <div className="text-slate-900 font-medium">{coach || "—"}</div>
            </div>
            <div>
              <div className="text-slate-500 text-sm">Players ({rosterText.split(/\n+/).filter(Boolean).length})</div>
            ...
            <div>
              ...
              <div className="whitespace-pre-wrap text-slate-900">{notes || "—"}</div>
            </div>
          </div>
        </Section>
      )}
...
function LadderView({ division, data }) {
  ...
  const ladder = useMemo(() => {
    ...
    return computeLadder(teams, fixtures, results);
  }, [teams.length, fixtures.length, results.length]);

  const neighbourhoods = useMemo(() => {
    if (teams.length === 0) return [];
    return computeNeighbourhoodTotals(teams, results, fixtures);
  }, [teams.length, results.length, fixtures.length]);

  return (
    <div className="grid gap-6">
      <Section title="Ladder">
        <Table
          caption={`League ladder for ${division} division`}
          columns={[
        ...
        {ladder.length === 0 && (
          <p className="text-slate-500 text-center py-8">
            No games played yet. Add some fixtures and results to see the ladder.
          </p>
        )}
      </Section>
      ...
      <Section title="Neighbourhood Totals">
        <Table
          caption={`Neighbourhood performance totals for ${division} division`}
          columns={[
            { key: "neighbourhood", header: "Neighbourhood" },
            { key: "Wins", header: "W" },
            { key: "Losses", header: "L" },
...
function AdminView({ division, data, onImportJSON, onPushAll }) {
  ...
  const exportJSON = useCallback(() => {
    ...
    try {
      ...
      a.href = url;
      a.download = `seepep-backup-${division}-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
  ...
  const importJSON = useCallback(async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(teams) || !Array.isArray(fixtures) || !Array.isArray(results)) {
  ...
  const handlePushAll = useCallback(async () => {
    if (!confirm("Push all teams, fixtures, and results to the backend? This will overwrite existing data.")) {
      return;
    }
    ...
    <div className="grid gap-6">
      <Section title="Backup & Restore">
        <div className="grid md:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <Button onClick={exportJSON} loading={loading.export}>
              Export JSON
            </Button>
          </div>
          ...
          <div className="space-y-2">
            ...
            <label className="inline-flex items-center gap-2 cursor-pointer">
              <input 
                ...
                onChange={importJSON}
                className="text-sm text-slate-500 file:mr-2 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
              />
            </label>
            {importError && (
          ...
          <div className="space-y-2">
            ...
            <Button variant="ghost" onClick={handlePushAll} loading={loading.push}>
              Push All to Sheets
            </Button>
          </div>
        </div>
      ...
      <Section title="Data Statistics">
        <div className="grid md:grid-cols-4 gap-4">
          <div className="bg-slate-50 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-slate-800">{data.teams.length}</div>
            <div className="text-sm text-slate-600">Teams</div>
          </div>
          <div className="bg-slate-50 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-slate-800">{data.fixtures.length}</div>
            <div className="text-sm text-slate-600">Fixtures</div>
          </div>
          <div className="bg-slate-50 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-slate-800">{data.results.length}</div>
            <div className="text-sm text-slate-600">Results</div>
          </div>
          <div className="bg-slate-50 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-slate-800">
              {data.fixtures.length > 0 ? Math.round((data.results.length / data.fixtures.length) * 100) : 0}%
            </div>
      ...
      <Section title="Debug Data">
        <details className="group">
          ...
          <pre className="mt-3 bg-slate-50 rounded-xl p-3 overflow-auto text-xs max-h-96">
            {JSON.stringify({ division, ...data }, null, 2)}
          </pre>
        </details>
      </Section>
    </div>
...
export default function App() {
  ...
  const setLoadingState = useCallback((type, message = "") => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        divisions: { ...s.divisions, [division]: { teams, fixtures, results } },
      }));
    } catch (e) {
    ...
    try {
      ...
      setState((s) => {
        ...
        return { 
          ...
          divisions: { 
            ...s.divisions, 
            [division]: { ...s.divisions[division], teams } 
          } 
        };
      });
    ...
    try {
      ...
      setState((s) => {
        ...
        const fixtures = exists 
          ? list.map((x) => x.id === fixture.id ? fixture : x) 
          : [...list, fixture];
        return { 
          ...s, 
          divisions: { 
            ...s.divisions, 
            [division]: { ...s.divisions[division], fixtures } 
          } 
        };
      });
    ...
    try {
      ...
      setState((s) => {
        ...
        return { 
          ...
          divisions: { 
            ...s.divisions, 
            [division]: { ...s.divisions[division], results } 
          } 
        };
      });
  ...
  const handleDeleteFixture = useCallback(async (id) => {
    if (!confirm("Delete this fixture and its result? This action cannot be undone.")) {
      return;
    }
    ...
    try {
      ...
      setState((s) => {
        ...
        return { 
          ...
          divisions: { 
            ...s.divisions, 
            [division]: { ...s.divisions[division], fixtures, results } 
          } 
        };
      });
  ...
  const handleImportJSON = useCallback((imported) => {
    setState((s) => ({ 
      ...s, 
      divisions: { ...s.divisions, [division]: imported } 
    }));
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-10 backdrop-blur bg-white/80 border-b border-slate-200">
        <div className="mx-auto max-w-6xl px-4 py-3 flex flex-wrap items-center gap-3">
          ...
          <Button 
            variant="ghost" 
            onClick={refresh} 
            loading={loading.type === "refresh"}
            ariaLabel="Refresh data"
          >
            {loading.type === "refresh" ? loading.message : "Refresh"}
          </Button>
        </div>
              ...
              >
                ...
                {t.count !== undefined && (
                  <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${
                    tab === t.id ? "bg-indigo-500" : "bg-slate-200 text-slate-600"
                  }`}>
                    {t.count}
                  </span>
                )}
        ...
        <div id={`panel-${tab}`} role="tabpanel">
          ...
          {tab === "fixtures" && (
            <FixturesView
              division={division}
              data={data}
              onUpsertFixture={handleUpsertFixture}
          ...
          {tab === "ladder" && (
            <LadderView division={division} data={data} />
          )}
          {tab === "admin" && (
            <AdminView
              division={division}
      ...
      <footer className="mx-auto max-w-6xl px-4 pb-8 text-xs text-slate-500">
        Backend: Google Apps Script · Frontend: React + Tailwind · 
        {loading.type && (
